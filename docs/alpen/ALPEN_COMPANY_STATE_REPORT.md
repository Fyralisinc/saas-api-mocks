...
## MODEL-LAYER ENRICHMENT APPLIED — 2026-06-13

The corpus now includes an explicit model-layer ground-truth layer:
`corpus/facts/company_truth.yaml`, rendered into normal provider events. It
adds belief-change arcs, opaque hallway/partner-pressure causes, handover
packets, Jira reassignment lag, and `org.person.depart` events. The detailed
answer key is `ALPEN_LABS_MODEL_LAYER_GROUND_TRUTH.md`.

Headcount should now be read as **42 historical identities / 29 active people
as of 2026-06-11**, not "42 active employees". Several short-tenure contributors
with stale `last_active` dates now have `ended_at`, so payroll, HR, Slack volume,
and future assignment behavior no longer treat them as current employees.
