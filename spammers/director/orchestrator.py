    ...
    async def _drain_once(self) -> int:
        ...
        for row in rows:
            try:
                ...
                elif etype == "hibob.event" and self._hibob_webhook_url:
                    await hibob_webhooks.emit_event(
                        self._pool,
                        run_id=self._run_id,
                        event_id=row["id"],
                ...
                elif etype == "ramp.transaction" and self._ramp_webhook_url:
                    await ramp_webhooks.emit_event(
                        self._pool,
                        run_id=self._run_id,
                        event_id=row["id"],
                        ramp_webhook_url=self._ramp_webhook_url,
                    )
                elif etype == "gusto.event" and self._gusto_webhook_url:
                    await gusto_webhooks.emit_event(
                        self._pool,
                        run_id=self._run_id,
                        event_id=row["id"],
